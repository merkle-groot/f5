...
describe("QuoteService.netFeeBPSNative bridge fee", () => {
  ...
  it("adds nothing for an OP-Stack destination (bridgeFee = 0)", async () => {
    const withZero = await svc.netFeeBPSNative(baseFee, balance, nativeQuote, gasPrice, 0n);
    const withDefault = await svc.netFeeBPSNative(baseFee, balance, nativeQuote, gasPrice);
  ...
  it("prices a fronted L1->L2 fee into the quote (Arbitrum/Starknet)", async () => {
    ...
    const withoutFee = await svc.netFeeBPSNative(baseFee, balance, nativeQuote, gasPrice, 0n);
    const withFee = await svc.netFeeBPSNative(baseFee, balance, nativeQuote, gasPrice, bridgeFeeWei);
