...
export async function relayQuoteHandler(
  ...
  try {
    quote = await quoteService.quoteFeeBPSNative({
      chainId, amountIn, assetAddress: asset, baseFeeBPS: config.fee_bps, bridgeFeeWei
    });
  } catch (e) {
    return next(e);
  }

  const { feeBPS, gasPrice, relayTxCost } = quote;
  ...
  if (recipient) {
    const feeReceiverAddress: `0x${string}` = getAddress(getFeeReceiverAddress(chainId));
    ...
    const relayerCommitment = { withdrawalData, expiration, asset, amount: amountIn };
    const signedRelayerCommitment = await web3Provider.signRelayerCommitment(chainId, relayerCommitment);
    quoteResponse.addFeeCommitment({ expiration, asset, withdrawalData, signedRelayerCommitment, amount: amountIn });
  }
